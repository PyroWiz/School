...
function updateClosedCounter() {
  const total = board.length * board.length;
  const opened = document.querySelectorAll('.cell.disabled').length;
  const closed = total - opened;
  const counter = document.getElementById('closed-counter');
  if (counter) counter.textContent = closed;
}
...
function animateFireballToCell(cell, callback)
{
  ...
  fire.style.left = `${startX - 8}px`; 
  fire.style.top = `${startY - 18}px`; 
...
function showSkyLightning(centerCell, callback)
{
  ...
  const targetY = rect.top + rect.height / 2 - 32; 
  ...
  bolt.style.setProperty('--targetY', `${targetY + 8}px`); 
...
function showVictoryScreen()
{
  ...
  const closed = document.getElementById('closed-counter')?.textContent;
  if (closed) {
    let msg = victory.querySelector('.closed-score');
    if (!msg) {
      msg = document.createElement('div');
      msg.className = 'closed-score';
      msg.style.fontSize = '1.3rem';
      msg.style.marginTop = '1.2rem';
      victory.appendChild(msg);
    }
    msg.textContent = `Closed tiles left: ${closed}`;
  }
}
...
function showMeteor(targetCell, callback)
{
  ...
  const targetX = rect.left + rect.width / 2 - 36; 
  const targetY = rect.top + rect.height / 2 - 36;

  const startX = window.innerWidth + 80;
  
  const deltaY = targetY + 36 + 120;  
  const deltaX = targetX - startX;
  const newDeltaY = deltaY * 0.8; 
